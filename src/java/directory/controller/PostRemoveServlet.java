 ...
 */
@Stateless
@WebServlet(name = "PostRemoveServlet", urlPatterns = {"/directory/post/remove"})
public class PostRemoveServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet PostRemoveServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet PostRemoveServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        post = postBean.find(Integer.parseInt(postId));
        
        try {
            postBean.remove(post);
            
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
        List<Object[]> obj = postBean.findPostWithUserAndNewsGroup();
        request.setAttribute("postList", obj);
