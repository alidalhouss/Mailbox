...
@Stateless
@WebServlet(name = "MyMailbox", urlPatterns = {"/mailbox-manager/mymailbox"})
public class MyMailbox extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet MyMailbox</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet MyMailbox at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        List<Message> messages = messageBean.findAllAUserMessage(user.getIdUser());
        request.setAttribute("messageList", messages);
        request.setAttribute("connectedAs", connectedUser.getUsername());
        getServletContext().getRequestDispatcher("/mailbox-manager/my-mailbox/view.jsp").forward(request, response);
        for (Message messageItem : messages) {
            messageItem.setIsRead(true);
            messageBean.edit(messageItem);
        }
    }
